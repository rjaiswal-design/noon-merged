...
function ProductRow({ name, src }: { name: string; src: string }) {
  return (
    <div className="flex w-full items-center gap-2">
      <div className="flex shrink-0 items-center rounded-8 bg-surface-tertiary p-1">
        <div className="relative size-10 shrink-0">
          <img src={src} alt="" className="block size-full object-cover" />
        </div>
      </div>
      <div className="flex min-w-0 flex-1 flex-col items-start">
        <p className="w-full overflow-hidden text-ellipsis whitespace-nowrap font-primary text-h16 font-normal leading-5 text-text-primary">
          {name}
        </p>
      </div>
    </div>
