...
function LetterKey({ children, width = 32 }: { children: string; width?: number }) {
  return (
    <div
      className="flex h-10 items-center justify-center rounded-4 bg-white shadow-[0px_1px_0px_0px_rgba(0,0,0,0.3)]"
      style={{ width }}
    >
      <span
        className="text-h20 leading-7 text-black"
...
function MetaKey({
  ...
  return (
    <div
      className="flex h-10 items-center justify-center rounded-4 shadow-[0px_1px_0px_0px_rgba(0,0,0,0.3)]"
...
function ShiftKey() {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-4 bg-white shadow-[0px_1px_0px_0px_rgba(0,0,0,0.3)]">
...
function DeleteKey() {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-4 bg-blue-gray-500 shadow-[0px_1px_0px_0px_rgba(0,0,0,0.3)]">
...
export default function IosKeyboardMock() {
  return (
    <div
      className="relative h-72 w-[375px] overflow-hidden"
      style={{ background: "#d1d3d9", backdropFilter: "blur(54px)" }}
    >
      {/* Row 1 */}
      <div className="absolute left-0.5 right-0.5 top-2 flex justify-between">
        {ROW_1.map((k) => (
      ...
      <div className="absolute left-5 right-5 top-16 flex justify-between">
        {ROW_2.map((k) => (
          <LetterKey key={k}>{k}</LetterKey>
        ))}
      </div>
      ...
      <div className="absolute left-0.5 right-0.5 top-[116px] flex items-start justify-between">
        <ShiftKey />
      ...
      <div className="absolute left-0.5 right-0.5 top-44 flex items-start gap-1.5">
        <MetaKey width={91}>123</MetaKey>
        <div className="flex h-10 flex-1 items-center justify-center rounded-4 bg-white shadow-[0px_1px_0px_0px_rgba(0,0,0,0.3)]">
          <span
            className="text-h16 text-black"
      ...
      {/* Emoji + dictation */}
      <div className="absolute left-8 right-8 top-60 flex items-start justify-between">
      ...
      <div className="absolute bottom-2 left-1/2 h-1 w-[134px] -translate-x-1/2 rounded-full bg-black" />
    </div>
  );
}
