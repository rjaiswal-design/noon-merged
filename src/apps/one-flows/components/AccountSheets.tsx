      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className={`absolute z-40 ${floating ? "left-3 right-3 bottom-4" : "left-0 right-0 bottom-0"}`}
          >
            <div
              className={`relative ${floating ? "rounded-16" : "rounded-tl-16 rounded-tr-16"} overflow-hidden`}
              style={{
...
function FlagKW({ className="" }: { className?: string }) {
  return (
    <div className={`${className} relative flex flex-col`} style={{ ...flagBase(), width: "28px", height: "20px" }}>
      ...
      <div style={{ backgroundColor: "#FFFFFF", flex: 1 }} />
      <div style={{ backgroundColor: "#CE1126", flex: 1 }} />
      <div className="absolute left-0 top-0 bottom-0 w-1.5" style={{ backgroundColor: "#000000", clipPath: "polygon(0 0, 100% 25%, 100% 75%, 0 100%)" }} />
    </div>
  );
}
...
function RadioEmpty() {
  return (
    <div
      className="size-5 rounded-full border shrink-0 bg-white"
...
function CheckCircle() {
  ...
  return (
    <svg viewBox="0 0 24 24" className="block size-6" fill="none" aria-hidden="true">
  ...
  return (
    <SheetShell open={open} onClose={onClose} floating>
      ...
      <div className="px-3 pt-0.5 pb-3 flex flex-col gap-3">
        ...
        <div className="bg-white rounded-16 px-2 py-3.5 flex flex-col gap-2 w-full">
          {COUNTRIES.map((c, i) => {
            ...
            return (
              <div key={c.code} className="flex flex-col gap-2">
                <button
                  ...
                  className="flex items-center gap-3 px-2 py-1 rounded-12 cursor-pointer w-full"
                >
                  <div className="size-5 w-7 flex items-center justify-center shrink-0">
                    {c.flag}
                  </div>
                  <p
                    className="flex-1 text-left font-noontree font-medium text-label-3p"
                    style={{ color: "#343d54" }}
                  >
                    {c.name}
                  </p>
                ...
                {i < COUNTRIES.length - 1 && (
                  ...
                  <div
                    className="h-0 border-t border-dashed ml-10"
...
function LogoutGlyph() {
  return (
    <svg viewBox="0 0 26 26" className="block size-6" fill="none" aria-hidden="true">
      <path
...
export function SignOutSheet({
  ...
  return (
    <SheetShell open={open} onClose={onClose}>
      <div className="px-5 pt-2.5 pb-7 flex flex-col items-center">
        ...
        {/* Title + body */}
        <p
          className="mt-4 font-noontree font-bold text-h18 text-center"
          style={{ color: T.color.text.deep }}
        >
          Leaving already?
        </p>
        <p
          className="mt-1.5 font-noontree text-label-3 text-center max-w-[280px]"
...
function HelpCard({ rows }: { rows: HelpRow[] }) {
  return (
    <SmoothCorners radius={12} className="bg-white rounded-12 p-3 flex flex-col">
      {rows.map((r, i) => (
        <div key={i} className="flex flex-col">
          <div className="flex items-center gap-2.5 py-2">
            <div className="size-7 flex items-center justify-center shrink-0">{r.icon}</div>
            <div className="flex-1 flex flex-col gap-0.5 min-w-0">
              <p
                className="font-noontree font-semibold text-b14"
                style={{ color: T.color.text.deep }}
              >
                {r.label}
              </p>
            ...
            <ChevronRightSm className="size-3.5 shrink-0" />
          </div>
          {i < rows.length - 1 && (
...
function HelpOrdersIcon({ color = T.color.text.heading }: { color?: string }) {
  // Shopping bag (Orders)
  return (
    <svg viewBox="0 0 24 24" className="block size-5" fill="none" aria-hidden="true">
...
export function NeedHelpSheet({
  ...
  return (
    <SheetShell open={open} onClose={onClose} floating>
      <div className="px-3 pt-0.5 pb-4 flex flex-col gap-3">
        <HelpCard
          rows={[
            {
              icon: <HelpOrdersIcon />,
...
function Toggle({ on, onChange }: { on: boolean; onChange: () => void }) {
  return (
    <button
      ...
      aria-checked={on}
      onClick={onChange}
      className="relative h-5 w-8 rounded-full cursor-pointer transition-colors duration-200"
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={() => onChange("en")}
        className="relative flex-1 rounded-full font-noontree font-semibold text-label-3p cursor-pointer transition-colors duration-150 z-10"
  ...
  return (
    <SheetShell open={open} onClose={onClose} floating>
      <div className="px-3 pt-0.5 pb-3 flex flex-col gap-3">
        ...
        <SmoothCorners radius={12} className="bg-white rounded-12 p-3 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex-1 flex flex-col gap-0.5 min-w-0">
              <p className="font-noontree font-bold text-b14" style={{ color: T.color.text.deep }}>
                Push notifications
              </p>
            ...
            {([
              ...
              <div key={row.label} className="flex flex-col">
                <div className="flex items-center justify-between py-2.5">
                  <p className="font-noontree font-medium text-label-3p" style={{ color: T.color.text.deep }}>
                    {row.label}
                  </p>
      ...
      <SheetShell open={open} onClose={onClose} floating>
        <div className="px-3 pt-0.5 pb-3 flex flex-col gap-3">
          <SmoothCorners radius={12} className="bg-white rounded-12 p-3 flex flex-col">
            ...
            {/* Dashed hairline aligned with text — matches AccountsPage. */}
            <div
              className="h-0 border-t border-dashed ml-9 my-1.5"
              style={{ borderColor: T.color.border.divider }}
            />
            ...
            <button
              ...
              className="flex items-center justify-between gap-2.5 py-1.5 cursor-pointer w-full"
            >
              <div className="flex items-center gap-2.5 flex-1 min-w-0">
                <div className="size-7 flex items-center justify-center shrink-0">
                  <TrashIcon />
      ...
      <div className="px-3 pt-0.5 pb-3 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between h-6 px-1">
          <button
            ...
            onClick={onClose}
            aria-label="Back"
            className="size-6 rounded-12 bg-white flex items-center justify-center cursor-pointer"
          >
            <svg viewBox="0 0 14 14" className="block size-3.5" fill="none" aria-hidden="true">
              <path d="M9 3.5L5.5 7L9 10.5" stroke={T.color.text.heading} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <p
            className="font-noontree font-bold text-h16 text-center"
            style={{ color: T.color.text.deep }}
          >
            {title}
          </p>
        ...
        <SmoothCorners radius={12} className="bg-white rounded-12 p-3">
          <p
            className="font-noontree text-label-4"
            style={{ color: T.color.text.body }}
          >
            {body}
          </p>
